...
class Comandos {
 private:
  ...
  static void escritaParalela(Diretrizes diretrizes[], int numArquivos, string filePath, int i, int th) {
  ...
  string importarArquivo(string caminhoComando, string caminhoArquivoImport) {
      ...
      if (fsys::exists(caminhoArquivoImport)) {
          ...
          for (int i = 0; i < numArquivos; ++i) {
              ...
              if (!fsys::exists(unidades[i] + "files/" + nomeDiretorio + "/")) {
                  fsys::create_directory(unidades[i] + "files/" + nomeDiretorio + "/");
              }
          }
          ...
          arqConfig
              << linhaConfig; //Adiciona o arquivo importado no arquivo de configuração (nomeArquivo;quantidadeArquivos)
          arqConfig.close();
